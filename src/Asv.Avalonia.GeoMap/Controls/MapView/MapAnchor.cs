    ...
    where TContext : class, IMapAnchor
{
    ...
    public MapAnchor(NavigationId id, ILoggerFactory loggerFactory, GeoPoint location)
        : base(id, loggerFactory)
    {
        Title = id.ToString();
        Polygon = new ObservableList<GeoPoint>();
        PolygonView = Polygon.ToNotifyCollectionChangedSlim().DisposeItWith(Disposable);
        Location = new BindableReactiveProperty<GeoPoint>(location).DisposeItWith(Disposable);
    }
    ...
    public BindableReactiveProperty<GeoPoint> Location { get; }
