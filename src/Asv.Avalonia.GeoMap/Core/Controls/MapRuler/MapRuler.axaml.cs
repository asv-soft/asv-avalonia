...
public sealed partial class MapRuler : UserControl
{
    ...
    {
        if (Map is not { } map || !map.Interaction.TryBegin(out var session))
        {
            return;
        }
        ...
        DistanceText = null;
        _session = session;
        Disposable.Create(OnSessionEnded).AddTo(session.Disposable);
        session.Clicked.Subscribe(OnSessionClick).AddTo(session.Disposable);
        session.CursorMoved.Subscribe(OnSessionCursor).AddTo(session.Disposable);

        _state = RulerState.PickingStart;
        session.Accent = AsvColorKind.Warning;
        session.Status = RS.MapRuler_PickStart;
    }
    ...
    private void OnSessionEnded()
    {
        ...
        if (_state is RulerState.PickingStart or RulerState.PickingEnd)
        {
            ClearRuler();
        }
    }
