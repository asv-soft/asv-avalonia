...
public static class AppInfoRegistrations
{
    extension(ServicesRegistrations.Builder builder)
    {
        public Builder AppInfo => new(builder);

        public ServicesRegistrations.Builder RegisterAppInfo(Action<Builder>? configure = null)
        {
            configure ??= x => x.UseDefault();
            var infoBuilder = new Builder(builder);
            configure(infoBuilder);
            builder.AppBuilder.Services.ReplaceSingleton(infoBuilder.Build());
            return builder;
        }
    }
