...
namespace Asv.Avalonia
{
    public static class CustomDialogInterface
    {
        ...
        {
            parent ??= Application.Current?.ApplicationLifetime
                is IClassicDesktopStyleApplicationLifetime desktop
                ? desktop.MainWindow
                : null;
            if (parent == null)
            {
                return (T?)ShowConsoleDialog(
                    title,
                    message,
                    isInputDialog,
                    defaultButton,
            ...
            var stackPanel = new StackPanel
            {
                ...
                Children =
                {
                    ...
                    isInputDialog
                        ? new TextBox { Margin = new Thickness(5) }
                        : new Control(),
                    new StackPanel
                    {
            ...
            ((StackPanel)stackPanel.Children[2]).Children.AddRange(
                CreateButtons(tcs, dialog, isInputDialog, defaultButton, buttonNames)
            );
            dialog.Content = stackPanel;

            dialog.Closed += (sender, e) => tcs.TrySetResult(default);
            ...
            await dialog.ShowDialog<T?>(parentWindow);
            var result = await tcs.Task;
            dialog.Close();
            return result;
        }

        private static List<Control> CreateButtons<T>(
            ...
            foreach (var name in buttonNames)
            {
                ...
                button.Click += (sender, e) =>
                {
                    ...
                    if (stackPanel != null)
                    {
                        tcs.TrySetResult(
                            isInputDialog
                                ? (T?)(object?)((TextBox)stackPanel.Children[1]).Text
                                : (T?)(object?)name
                        );
                    }
                    else
        ...
        private static object? ShowConsoleDialog(
            string title,
            string message,
            bool isInputDialog,
            string defaultButton,
