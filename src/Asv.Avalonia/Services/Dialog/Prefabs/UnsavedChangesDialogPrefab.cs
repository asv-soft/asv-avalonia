...
public sealed class UnsavedChangesDialogPayload
{
    ...
    /// </summary>
    public required List<UnsavedChangeMeta> Changes { get; init; }
}
    ...
    : IDialogPrefab<UnsavedChangesDialogPayload, bool>
{
    ...
    {
        using var vm = new DialogItemUnsavedChangesViewModel(
            loggerFactory,
            nav,
            dialogPayload.Changes
        );

        var dialogContent = new ContentDialog(vm, nav)
        {
            Title = RS.DesktopShellViewModel_CloseAsync_UnsavedConfirm,
