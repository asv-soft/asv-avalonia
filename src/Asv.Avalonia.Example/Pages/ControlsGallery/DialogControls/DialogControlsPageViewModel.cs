...
public class DialogControlsPageViewModel : ControlsGallerySubPage
{
        ...
        : base(PageId, loggerFactory)
    {
        ...
        OpenPositionDialogCommand = new ReactiveCommand(ShowPositionDialog).DisposeItWith(
            Disposable
        );
    }
    ...
    private async ValueTask ShowPositionDialog(Unit unit, CancellationToken cancellationToken)
    {
        ...
        {
            StartPosition = GeoPointProperty.ModelValue.CurrentValue,
        };

        var rawResult = await _positionDialog.ShowDialogAsync(payload);
        var result = rawResult?.ToString() ?? $"({RS.DialogControlsPageViewModel_CancelResult})";
        var msg = string.Format(RS.DialogControlsPageViewModel_Position_Result, result);
        LastResult.OnNext(msg);
        Logger.LogInformation("{msg}", msg);
